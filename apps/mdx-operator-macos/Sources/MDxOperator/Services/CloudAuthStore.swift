...
struct CloudConfiguration: Equatable, Sendable {
  ...
  static var current: CloudConfiguration? {
    func value(_ environment: String, _ plist: String) -> String {
      if let raw = ProcessInfo.processInfo.environment[environment]?.trimmingCharacters(in: .whitespacesAndNewlines), !raw.isEmpty {
        return raw
      }
      return String(Bundle.main.object(forInfoDictionaryKey: plist) as? String ?? "")
        .trimmingCharacters(in: .whitespacesAndNewlines)
    }
    guard let supabaseURL = URL(string: value("MDX_SUPABASE_URL", "MDXSupabaseURL")),
          let hostedOrigin = URL(string: value("MDX_PUBLIC_HOST_URL", "MDXPublicHostURL")),
          supabaseURL.scheme == "https",
          hostedOrigin.scheme == "https" else { return nil }
    ...
    return CloudConfiguration(
      ...
      redirectURL: URL(string: "mdx-workbench://auth/callback")!
    )
  }
}
@MainActor
@Observable
final class CloudAuthStore {
  ...
  init(configuration: CloudConfiguration? = .current) {
    ...
    if let configuration {
      client = SupabaseClient(
        ...
        options: SupabaseClientOptions(
          auth: .init(
            storage: KeychainLocalStorage(service: "com.mdx.app.supabase"),
  ...
  private func applyIdentity(_ user: User, preferredName: String? = nil) {
    ...
    displayName = preferredName?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false
      ? preferredName!
      : candidate ?? emailName
  }
